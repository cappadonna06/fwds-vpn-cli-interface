...
| Cellular | no modem detected (hardware fault) | **red** issue; triggered by the modem's own `No cellular modems detected` output (`cell-status`/`cell-imei`) or an unenumerated modem (`-65552`, missing `/dev/ttyUSB2`, missing `wwan0`); action `Reboot controller — if modem still not detected, hardware service needed` ([parsers.rs](/Users/marcsells/Developer/FWDS-VPN-CLI-INTERFACE/src-tauri/src/parsers.rs:5319)) | not modeled separately |
| Cellular | modem not responding (AT interface dead) | red issue with reboot / reseat guidance; fires only when the modem answered its identity (IMEI read) but the AT control channel failed — **not** on a `setup-cellular` connection timeout, which a healthy modem in a no-coverage area produces identically ([parsers.rs](/Users/marcsells/Developer/FWDS-VPN-CLI-INTERFACE/src-tauri/src/parsers.rs:5337)) | not modeled separately |
