...
const RECOMMENDED_ACTION_TOPICS = [
  "Ethernet",
  "Wi-Fi",
  "Cellular",
  "Satellite",
...
type RecommendedActionTopic = (typeof RECOMMENDED_ACTION_TOPICS)[number];

// ── SlackPreview ──────────────────────────────────────────────────────────────

export function SlackPreview({ report }: { report: SessionReport }) {
  const visibleActions = report.actions.filter(a => a.text.trim());
  const visibleRecs = report.recommendedActions.filter(a => a.text.trim());
...
const QUICK_ACTIONS = (version: string, systemType?: string | null, zoneCount?: number | null) => [
  { label: "Upgraded firmware", text: `Upgraded firmware to ${version || "(x.x)"}` },
  { label: "Configured zones",  text: `System configured: ${systemType?.trim() || "(type)"} · ${zoneCount ?? "(x)"} zones` },
...
type IfacePreset = {
  ...
  recommendedAction?: {
    interface: RecommendedActionTopic;
    text: string;
    detail?: string;
  };
};
...
/** Record a generated key as user-owned so diagnostics polling stops re-adding it. */
function addDismissedKey(keys: string[], key?: string): string[] {
...
export default function ReportTab() {
  ...
  const [recTopicMenuOpen, setRecTopicMenuOpen] = useState(false);
  const lastEditAtRef = useRef(0);
  const previewReport = useDeferredValue(report);
  const reportRef = useRef(report);
  reportRef.current = report;
  const visibleActions = report.actions;
  const visibleRecommendedActions = report.recommendedActions;

  function mkLocalId(prefix: string) {
    return `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  }

  function createManualRecommendedAction(topic: RecommendedActionTopic): ReportRecommendedAction {
    return {
      id: mkLocalId(`rec-${topic.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`),
      interface: topic,
      text: "",
      detail: "",
      checked: false,
  ...
  async function fetchAndUpdate() {
    ...
    try {
      ...
      setReport(prev => {
        // Rows the user removed or edited are theirs now: drop them from the generated
        // set so they are neither refreshed nor re-appended as a fresh duplicate below.
        const dismissedActionKeys = new Set(prev.dismissedActionKeys);
        const dismissedRecKeys = new Set(prev.dismissedRecommendedKeys);
        const liveActions = actions.filter(a => !(a.key && dismissedActionKeys.has(a.key)));
        const liveRecs = recommendedActions.filter(
          a => !(a.generatedKey && dismissedRecKeys.has(a.generatedKey))
        );

        // Merge actions: for auto-generated ones match by stable key so refreshes replace the same row.
        const mergedGeneratedActions = liveActions.map(a => {
          if (!a.autoGenerated || !a.key) return a;
          const prevMatch = prev.actions.find(pa => pa.key === a.key);
          return prevMatch ? { ...a, id: prevMatch.id } : a;
        });
        ...
        // Merge generated recommended actions by stable key and keep manual rows intact.
        const prevRecMap = new Map(
          prev.recommendedActions
            .filter((a) => !!a.generatedKey)
            .map((a) => [a.generatedKey as string, a])
        );
        const mergedGeneratedRecs = liveRecs.map(a => {
          const p = a.generatedKey ? prevRecMap.get(a.generatedKey) : undefined;
          return p ? { ...a, id: p.id, checked: p.checked } : a;
        });
        ...
        const mergedRecs = [
          ...prev.recommendedActions.flatMap((action) => {
            if (!action.generatedKey) return [action];
            const key = action.generatedKey;
          ...
          ...mergedGeneratedRecs.filter((action) => {
            if (!action.generatedKey) return false;
            return !usedGeneratedRecKeys.has(action.generatedKey);
          }),
        ];
        ...
        return {
          ...
          location: diagState.system?.location?.trim() ?? "",
  ...
  function addAction() {
    ...
    setReport(r => ({
      ...r,
      actions: [...r.actions, { id: mkLocalId("action"), text: "" }],
    }));
  }

  function addRec(topic: RecommendedActionTopic) {
    lastEditAtRef.current = Date.now();
    setReport(r => ({
      ...r,
      recommendedActions: [...r.recommendedActions, createManualRecommendedAction(topic)],
    }));
    setRecTopicMenuOpen(false);
  }

  function removeAction(id: string) {
    lastEditAtRef.current = Date.now();
    setReport(r => {
      const removed = r.actions.find((a) => a.id === id);
      return {
        ...r,
        actions: r.actions.filter((a) => a.id !== id),
        dismissedActionKeys: addDismissedKey(
          r.dismissedActionKeys,
          removed?.autoGenerated ? removed.key : undefined,
        ),
      };
    });
  }
  ...
  function removeRec(id: string) {
    lastEditAtRef.current = Date.now();
    setReport(r => {
      const removed = r.recommendedActions.find((a) => a.id === id);
      return {
        ...r,
        recommendedActions: r.recommendedActions.filter((a) => a.id !== id),
        dismissedRecommendedKeys: addDismissedKey(r.dismissedRecommendedKeys, removed?.generatedKey),
      };
    });
  }
  ...
  function updateActionText(id: string, text: string) {
    ...
    setReport(r => {
      ...
      const actions = r.actions.map(a => {
        if (a.id !== id) return a;
        // Editing an auto-generated row hands it to the user — cut the generator
        // link so the next poll can't overwrite what they typed.
        if (a.autoGenerated && a.key && text !== a.text) {
          claimedKey = a.key;
          return { ...a, text, key: undefined, autoGenerated: false };
        }
        return { ...a, text };
      });
      return {
        ...r,
        actions,
        dismissedActionKeys: addDismissedKey(r.dismissedActionKeys, claimedKey),
      };
    });
  }
  ...
  function updateRec(id: string, patch: Partial<ReportRecommendedAction>) {
    ...
    setReport(r => {
      let claimedKey: string | undefined;
      const recommendedActions = r.recommendedActions.map((action) => {
        if (action.id !== id) return action;
        const contentEdited =
          ("text" in patch && patch.text !== undefined && patch.text !== action.text)
          || ("detail" in patch && patch.detail !== undefined && patch.detail !== action.detail);
        if (contentEdited && action.generatedKey) {
          claimedKey = action.generatedKey;
          return {
            ...action,
            ...patch,
      ...
      return {
        ...r,
        recommendedActions,
        dismissedRecommendedKeys: addDismissedKey(r.dismissedRecommendedKeys, claimedKey),
      };
    });
  }
  ...
  function moveRecommendedAction(id: string, offset: -1 | 1) {
    lastEditAtRef.current = Date.now();
    setReport((current) => {
      const visible = current.recommendedActions;
      const index = visible.findIndex((action) => action.id === id);
      const target = index + offset;
      if (index === -1 || target < 0 || target >= visible.length) return current;
  ...
  const quickActions = useMemo(
    () => QUICK_ACTIONS(report.version, diagSnapshot?.system?.system_type, diagSnapshot?.system?.zone_count),
    [report.version, diagSnapshot?.system?.system_type, diagSnapshot?.system?.zone_count]
  );
      ...
      <div className="report-header">
        <div className="report-header-left">
          ...
          <span className="report-header-sub">
            {report.generated
              ? [report.sid, report.version, report.location, report.ip, report.date].filter(Boolean).join(" · ")
          ...
          <div className="report-edit-pane">
            ...
            <div className="report-card">
              ...
              {visibleActions.map((action, index) => (
                <div key={action.id} className="report-action-row">
                  ...
                  />
                  <button className="report-dismiss-btn" onClick={() => removeAction(action.id)}>✕</button>
                </div>
              ))}
                ...
                {quickActions.map(qa => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setReport(r => ({
                        ...r,
                        actions: [...r.actions, { id: mkLocalId("action"), text: qa.text }],
                      }));
                    }}
                  >
                    + {qa.label}
            ...
            <div className="report-card">
              <div className="report-section-header">
                ...
                <div className="report-add-menu-wrap">
                  <button className="btn-link" onClick={() => setRecTopicMenuOpen((open) => !open)}>+ Add</button>
                  {recTopicMenuOpen && (
                    <div className="report-add-menu">
                      {RECOMMENDED_ACTION_TOPICS.map((topic) => (
                        <button
                          key={topic}
                          type="button"
                          className="report-add-menu-item"
                          onClick={() => addRec(topic)}
                        >
                          {IFACE_ICON[topic] ?? ""} {topic}
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div key={action.id} className="report-rec-row">
                  ...
                  <div className="report-rec-content">
                    <span className="report-rec-iface">
                      ...
                      {!action.custom && action.generatedKey ? " — Auto" : ""}
                    </span>
                    <input
                    ...
                    />
                    <textarea
                      className="report-rec-detail-input"
                      rows={2}
                      value={action.detail}
                      placeholder="Detail (optional)…"
                      onChange={e => updateRec(action.id, { detail: e.target.value })}
                    />
                  </div>
                  <button className="report-dismiss-btn" onClick={() => removeRec(action.id)}>✕</button>
                </div>
              ))}
            </div>
