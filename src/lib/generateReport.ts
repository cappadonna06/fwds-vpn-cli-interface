...
export function generatePressureRows(diag: DiagnosticState): PressureStatusRow[] {
  ...
  const PRESSURE_SOURCE_ZERO_EQUIVALENT_MIN = -10;
  ...
  const hasReportableSource = (v?: number | null) =>
    v !== null
    && v !== undefined
    && v <= 218
    && (v >= 0 || v >= PRESSURE_SOURCE_ZERO_EQUIVALENT_MIN);
  const hasReportableDistribution = (v?: number | null) =>
    v !== null
    && v !== undefined
    && v <= 218
    && (v >= 0 || Math.abs(v) < PRESSURE_NEAR_ZERO_DISPLAY_THRESHOLD);
  const formatSource = (v: number) =>
    v < 0 ? "~0.0 PSI" : `${v.toFixed(2)} PSI`;
  ...
  if (hasReportableSource(source)) readingParts.push(`P3 ${formatSource(source!)}`);
...
export function generateRecommendedActions(
  ...
  let id = 0;
  const mkId = () => String(++id);
  const pushGeneratedAction = (
    iface: ReportRecommendedAction["interface"],
    generatedKey: string,
    text: string,
    detail: string,
  ) => {
  ...
  if (diag.ethernet) {
    ...
    if (eth.status === "red" && (eth.flap_count ?? 0) > 3) {
      pushGeneratedAction(
        "Ethernet",
        "ethernet:swap-switch-port",
        "Swap switch port",
        `Link flapping detected — ${eth.flap_count} events. Force switch port speed or move to different port.`,
      );
    } else if (eth.status === "red" && eth.link_detected === false) {
      pushGeneratedAction(
        "Ethernet",
        "ethernet:check-cable-switch-port",
        "Check cable and switch port",
        "No physical link detected. Verify cable is seated and switch port is active.",
      );
    } else if (eth.status === "orange") {
      pushGeneratedAction(
        "Ethernet",
        "ethernet:check-router-upstream",
        "Check router upstream connectivity",
        "Link is up but DNS or internet is failing. Issue is likely upstream of the controller.",
      );
    }
  }
  ...
  if (diag.wifi) {
    ...
    if (wifi.status === "red" && wifi.check_result === "Failure") {
      pushGeneratedAction(
        ...
        "wifi:configure",
        "Configure Wi-Fi",
        "Re-run setup-wifi and configure to verified SSID and password.",
      );
    } else if (wifi.status === "red" && (wifi.strength_score ?? 100) < 40) {
      pushGeneratedAction(
        "Wi-Fi",
        "wifi:relocate-router",
        "Relocate router or add repeater",
        `Signal critically weak — ${wifi.strength_score}/100. Controller may lose Wi-Fi intermittently.`,
      );
    } else if (
      wifi.status === "orange" &&
      ...
      pushGeneratedAction(
        ...
        `Signal ${wifi.strength_score ?? "?"}/100 (${wifi.strength_label ?? "weak"}). Move AP closer or add a repeater to improve reliability.`,
      );
    }
  }
  ...
  if (diag.cellular) {
    ...
    if (cell.modem_unreachable) {
      pushGeneratedAction(
        "Cellular",
        "cellular:reboot-modem-unreachable",
        "Reboot controller — modem not responding",
        "Hardware detected but AT interface failed. Reboot resolves this in most cases. If issue persists after reboot, reseat the modem.",
      );
      ...
      if (cell.modem_not_present) {
        pushGeneratedAction(
          "Cellular",
          "cellular:check-modem-seating",
          "Check modem connection / seating",
          "No modem detected. Verify modem board is seated, then reboot controller.",
        );
      }
      ...
      if (noService && cell.sim_inserted === false) {
        pushGeneratedAction(
          "Cellular",
          "cellular:check-sim-seating",
          "Check SIM card is seated correctly",
          "Modem detected but SIM not found. Reseat SIM or try a known-good SIM.",
        );
        ...
        if (sp?.scan_completed && !sp.scan_empty) {
          ...
          if (typeof rec === "object" && "SwapTo" in rec) {
            pushGeneratedAction(
              "Cellular",
              `cellular:install-sim:${rec.SwapTo.toLowerCase()}`,
              `Install ${rec.SwapTo} SIM`,
              sp.recommendation_detail,
            );
          } else if (sp.scan_empty) {
            pushGeneratedAction(
              "Cellular",
              "cellular:no-carrier-coverage",
              "No carrier coverage at this location",
              "SIM Picker found no detectable carriers. Check antenna and sky view.",
            );
          }
        } else {
          pushGeneratedAction(
            "Cellular",
            "cellular:check-coverage-antenna",
            "Check coverage area and antenna",
            "SIM detected but no network service. Verify antenna connection and site coverage.",
          );
          pushGeneratedAction(
            "Cellular",
            "cellular:reboot",
            "Reboot controller",
            "After checking coverage and antenna, reboot and retry cellular diagnostics.",
          );
        }
      } else if (cell.status === "orange" && (cell.strength_score ?? 100) < 50) {
        pushGeneratedAction(
          "Cellular",
          "cellular:check-antenna-placement",
          "Check antenna connection and placement",
          `Signal ${cell.strength_score}/100. Verify antenna is hand-tight and not obstructed.`,
        );
      }
    }
  }
  ...
  if (diag.satellite) {
    ...
    const addSatelliteAction = (text: string, detail = "") => {
      ...
      pushGeneratedAction(
        "Satellite",
        `satellite:${text.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "")}`,
        text,
        detail,
      );
    };
  ...
  if (diag.pressure?.issues?.length) {
    for (const issue of diag.pressure.issues) {
      pushGeneratedAction(
        ...
        `pressure:${issue.id}`,
        issue.title,
        `${issue.description} ${issue.action}`.trim(),
      );
    }
  }
...
export function formatSlack(report: SessionReport): string {
  const lines: string[] = [];
  const headerParts = [report.sid || "—", report.version || "—"];
  if (report.location?.trim()) headerParts.push(`*${report.location.trim()}*`);

  lines.push(`*Controller Session — ${report.date}*`);
  lines.push(`*SID:* ${headerParts.join(" · ")}`);
  lines.push("");
...
export function formatSlackHtml(report: SessionReport): string {
  ...
  const headerParts = [escapeSlackHtml(report.sid || "—"), escapeSlackHtml(report.version || "—")];
  if (report.location?.trim()) headerParts.push(`<i>${escapeSlackHtml(report.location.trim())}</i>`);

  pushLine(`<b>${escapeSlackHtml(`Controller Session — ${report.date}`)}</b>`);
  pushLine(`<b>SID:</b> ${headerParts.join(" · ")}`);
  pushLine();
...
export function formatJira(report: SessionReport): string {
  const lines: string[] = [];
  const metaParts = [`*SID:* ${report.sid || "—"}`, `*Firmware:* ${report.version || "—"}`];
  if (report.location?.trim()) metaParts.push(`*Location:* ${report.location.trim()}`);
  metaParts.push(`*IP:* ${report.ip || "—"}`);
